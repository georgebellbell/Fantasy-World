...
CubeRobot::CubeRobot(Shader* shader, Mesh* cube) : SceneNode(shader) {
	SceneNode* body = new SceneNode(shader, cube, Vector4(1, 0, 0, 1));
	...
	SceneNode* leftLeg = new SceneNode(shader, cube, Vector4(0, 0, 1, 1));
	leftLeg->SetModelScale(Vector3(3, -17.5, 3));
	leftLeg->SetTransform(Matrix4::Translation(Vector3(-8,0,0)));
	body->AddChild(leftLeg);

	SceneNode* rightLeg = new SceneNode(shader, cube, Vector4(0, 0, 1, 1));
